...
class Encoder(nn.Module):
    ...
    def forward(self, src, hidden=None, self_mask=None):
        if self.tf:
            src = self.dropout(self.pos_enc(src))
            for attn_layer in self.layers:
                src, attn = attn_layer(src, self_mask)
            return src, attn
...
class Decoder(nn.Module):
    ...
    def forward(self, tgt, hidden=None, src_out=None, self_mask=None, cross_mask=None):
        if self.tf:
            tgt = self.dropout(self.pos_enc(tgt))
            for attn_layer in self.layers:
                tgt, attn = attn_layer(tgt, src_out, self_mask, cross_mask)
            return tgt, attn
        else:
            for layer in self.layers:
...
class ATTNLayer(nn.Module):
    ...
    def forward(self, seq, memory=None, self_mask=None, cross_mask=None):

        if self.cross:
            output, _ = self.mattn(seq, seq, seq, attn_mask=self_mask)
            output = output + self.norm(output)
            
            output, weight = self.cross_mattn(seq, memory, memory, attn_mask=cross_mask)
            output = output + self.norm(output)

            output = self.pos_ff(output) 
        else:
            output, weight = self.mattn(seq, seq, seq, attn_mask=self_mask)
            output = output + self.norm(output)
